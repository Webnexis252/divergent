...
export function CourseCurriculumSection({
  ...
  // Extract all unique months from the items
  const months = useMemo(() => {
    ...
    // Sort months (you could parse them to sort properly, but for simplicity we'll just sort alphabetically or keep insertion order if it's mostly chronological)
    return Array.from(monthSet).sort((a, b) => new Date(a).getTime() - new Date(b).getTime());
  }, [liveClasses, assignments, tests]);

  const filteredLiveClasses = useMemo(() => {
    if (selectedMonth === "ALL") return liveClasses;
    return liveClasses.filter(lc => new Date(lc.startTime).toLocaleString('default', { month: 'long', year: 'numeric' }) === selectedMonth);
  }, [liveClasses, selectedMonth]);

  const filteredAssignments = useMemo(() => {
    if (selectedMonth === "ALL") return assignments;
    return assignments.filter(a => new Date(a.createdAt).toLocaleString('default', { month: 'long', year: 'numeric' }) === selectedMonth);
  }, [assignments, selectedMonth]);

  const filteredTests = useMemo(() => {
    if (selectedMonth === "ALL") return tests;
    return tests.filter(t => new Date(t.createdAt).toLocaleString('default', { month: 'long', year: 'numeric' }) === selectedMonth);
  }, [tests, selectedMonth]);

  const hasContent = chapters.length > 0 || filteredLiveClasses.length > 0 || filteredAssignments.length > 0 || filteredTests.length > 0;
    ...
    <div className="space-y-6">
        ...
        {months.length > 0 && (
          <div className="relative">
            <select
              ...
              className="appearance-none bg-white border border-gray-200 rounded-lg pl-10 pr-8 py-2 text-sm font-medium text-gray-700 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors cursor-pointer"
            >
      ...
      <div className="space-y-4">
        {hasContent ? (
          <>
            {/* Course Modules */}
            {chapters.length > 0 && selectedMonth === "ALL" && (
