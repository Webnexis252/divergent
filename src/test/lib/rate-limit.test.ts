...
describe('checkRateLimit', () => {
  ...
  it('blocks requests exceeding the limit', async () => {
    ...
    await checkRateLimit(req, opts); // 1
    await checkRateLimit(req, opts); // 2
    await checkRateLimit(req, opts); // 3 — limit reached
    const result = await checkRateLimit(req, opts); // 4 — over limit
  ...
  it('counts remaining correctly', async () => {
    ...
    await checkRateLimit(req, opts); // 1 used
    const result = await checkRateLimit(req, opts); // 2 used
  ...
  it('isolates different IPs', async () => {
    ...
    await checkRateLimit(reqA, opts);
    const resultA = await checkRateLimit(reqA, opts);

    const resultB = await checkRateLimit(reqB, opts);

    expect(resultA.success).toBe(false); // A is over limit
    expect(resultB.success).toBe(true);  // B is fresh
  });

  it('returns correct limit and resetAt metadata', async () => {
